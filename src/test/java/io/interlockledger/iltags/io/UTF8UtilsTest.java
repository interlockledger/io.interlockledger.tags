...
public class UTF8UtilsTest {
	...
	private static final byte [][] WIKI_CODEPOINTS_UTF8 = {
			{(byte)0x24},
			{(byte)0xC2, (byte)0xA2},
	...
	public void testToCodepointWiki() {
		...
		for (int i = 0; i < WIKI_CODEPOINTS.length; i++) {
			assertEquals(WIKI_CODEPOINTS[i], UTF8Utils.toCodepoint(WIKI_CODEPOINTS_UTF8[i], WIKI_CODEPOINTS_UTF8[i].length));
		}
	}
